package com.example.mac.puzzlegame.puzzleGame.util;
...
public class PuzzleImageUtil {
     ...
     */
    public static final Bitmap getSubdivisionOfBitmap(Bitmap bitmap, int subdivisionWidth, int subdivisionHeight, int subdivisionRowIndex, int subdivisionColumnIndex)throws IllegalArgumentException{
        ...
        if((xStart+subdivisionWidth) > bitmapWidth || (yStart+subdivisionHeight) > bitmapHeight){
            throw new IllegalArgumentException("Cannot get subdivision outside bounds of the image width and height");
        }
        bitmap.getPixels(pixels, 0,subdivisionWidth, xStart,yStart,subdivisionWidth, subdivisionHeight);
        return Bitmap.createBitmap(pixels,subdivisionWidth,subdivisionHeight,Bitmap.Config.ARGB_8888);
    }
}
